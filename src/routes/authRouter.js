  ...
  .post(async (req, res) => {
    ...
    try {
      const [newUser, isCreated] = await User.findOrCreate({
        where: { email },
        defaults: { name, email, pass: hashPassword },
      });
      if (!isCreated) return res.sendStatus(400);
      req.session.user = {
        id: newUser.id, name: newUser.name, email: newUser.email, password: newUser.password,
      };
      res.json({
        id: newUser.id, name: newUser.name, email: newUser.email, password: newUser.password,
      });
...
authRouter.route('/')
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      const isValid = await compare(password, userFromDb.pass);
      ...
      req.session.user = ({
        id: userFromDb.id, name: userFromDb.name, email: userFromDb.email, password: userFromDb.password,
      });
      res.json({
        id: userFromDb.id, name: userFromDb.name, email: userFromDb.email, password: userFromDb.password,
      });
